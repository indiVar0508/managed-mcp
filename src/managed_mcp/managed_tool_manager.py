"""ManagedToolManager is responsible for detecting and loading tool functions from a specified directory."""
import ast
import importlib
import logging
import os
...
class ManagedToolManager(ToolManager):
    """
    ManagedToolManager is responsible for detecting and loading tool functions from a specified directory.
    ...
    Example:
        >>> from managed_mcp.tool_manager import ManagedToolManager
        >>> tool_manager = ManagedToolManager("/path/to/tools")
        >>> tool_manager.load()  # Loads all tool functions
        >>> tool_functions = tool_manager.live_tools  # Access loaded tool functions.

    """

    def __init__(self, *, tool_dir: str, **kwargs):  # noqa: D107
        ...
        logger.info(f"ManagedToolManager initialized with tool directory: {self.tool_dir}")
        self.live_tools = {}
